...
def get_model_output(model, device, data_loader, inputs, winding_number):
    y_pred = []
    with torch.no_grad():
...
def main():
    ...
    for filename in os.listdir(folder_path):
        ...
        if match:
            Np = int(match.group(1)) #TODO make sure whether the first number present Np
            Ns = int(match.group(2)) #TODO make sure whether the second number present Ns
    ...
    loss_IW_Ls = get_model_output(model_IW_Ls, device, data_loader_IW, inputs, 1)
    loss_IW_Lp = get_model_output(model_IW_Lp, device, data_loader_IW, inputs, 0)
    loss_OW_Ls = get_model_output(model_OW_Ls, device, data_loader_OW, inputs, 1)
    loss_OW_Lp = get_model_output(model_OW_Lp, device, data_loader_OW, inputs, 0)
    ...
    for i in range(0, np.shape(inputs)[0]):
        for j in range(0,6):
                if j < inputs[i,1]:
                    corner_radius_Ls[i,j] = inputs[i,7] + (j+1)*(inputs[i,4]+ 1) #TODO make sure the winding width
    ...
    for i in range(0, np.shape(inputs)[0]):
        for j in range(0,6):
                if j < inputs[i,0]:
                    corner_radius_Lp[i,j] = inputs[i,7] + j*(inputs[i,5]+ 1) + inputs[i,1]*(inputs[i,4]+ 1) + inputs[i,6] #TODO make sure the winding length
    ...
    rest = (section_loss_IW_Ls + section_loss_IW_Lp + section_loss_OW_Ls + section_loss_OW_Lp)*2 + corner_loss_OW_Lp + corner_loss_OW_Ls
    coef = (rest - corner_loss_OW_Ls -corner_loss_OW_Lp) / ((corner_loss_IW_Lp + corner_loss_IW_Ls) - (corner_loss_OW_Lp + corner_loss_OW_Ls))
