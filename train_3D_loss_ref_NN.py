...
LOG_FILE = "results_coef/train_3D_loss_NN.txt"
MODEL_FILE = "results_coef/Model_3D_loss_NN.pth"
ERROR_FILE = "results_coef/train_3D_error_loss_NN.csv"
...
data_length = 1400 #! train data number
...
def get_dataset(adr):
    ...
    # pre-process
    inputs = df.iloc[:14, 0:data_length].values
    inputs[:2] = inputs[:2]/10
    inputs[2:] = inputs[2:]
    outputs = df.iloc[14:15, 0:data_length].values #! loss 14:15, coef 23:24
    ...
    print(outputs_max)
    print(outputs_min)
    outputs_max = np.array([-1.4])
    outputs_min = np.array([-3.1])
